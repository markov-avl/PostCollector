...
from src.markup.callbackdata import CancelUnsubscribeCallbackData
...
@component
class OnCancelUnsubscribeHandler(BotEventHandler):
    ...
    def filters(self) -> list[Any]:
        return [CancelUnsubscribeCallbackData.filter()]

    def type(self) -> BotHandlerType:
        return BotHandlerType.CALLBACK_QUERY

    async def handle(self, callback_query: types.CallbackQuery, callback_data: CancelUnsubscribeCallbackData) -> None:
        logger.debug("CancelUnsubscribe from {}", callback_query.from_user.username)
        ...
        await callback_query.message.edit_text(text="Отписка от каналов отменена")
