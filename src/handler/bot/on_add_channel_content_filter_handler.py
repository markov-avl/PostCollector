...
class OnAddChannelContentFilterHandler(BotEventHandler):
    ...
    #
    #     1. `hashtags(*args)` - предикат, проверяющий есть ли в посте канала соответствующие хештеги. Если не указаны аргументы, то проверяется, что в посте есть хотя бы один хештег
    #     *Пример*: `hashtags(реклама, рассылка)`, `hashtags()`
    #
    #     2. `links(*args)` - предикат, проверяющий есть ли в посте канала соответствующие ссылки. Если не указаны аргументы, то проверяется, что в посте есть хотя бы одна ссылка
    #     *Пример*: `links(https://t.me/durov)`, `links()`
    #
    #     Примеры выражений:
    #     - `hashtags(реклама) | links()` - отбираются посты, в которых есть хештег #реклама или ссылки
    #     - `!(links() & hashtags(полезно, важно))` - фильтруются посты, в которых есть ссылки и есть хештеги #полезно или #важно
    #
    ...
    INSTRUCTION = """
            ...
            Предикаты для использования в выражении:
            1. `links(*args)` - предикат, проверяющий есть ли в посте все соответствующие ссылки. Если аргументы не указаны, то проверяется, что в посте есть хотя бы одна ссылка
            *Пример*: `links()`, `links(https://t.me/durov)`

            *Примечания*:
            - пробелы в выражении никак не учитываются;
            - разделителем аргументов является запятая.
        """
