...
from src.markup.callbackdata import AcceptUnsubscribeCallbackData
...
@component
class OnAcceptUnsubscribeHandler(BotEventHandler):
    ...
    def filters(self) -> list[Any]:
        return [AcceptUnsubscribeCallbackData.filter()]

    def type(self) -> BotHandlerType:
        return BotHandlerType.CALLBACK_QUERY

    async def handle(self, callback_query: types.CallbackQuery, callback_data: AcceptUnsubscribeCallbackData) -> None:
        logger.debug("AcceptUnsubscribe from {}", callback_query.from_user.username)
